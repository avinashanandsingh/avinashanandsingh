...
export class UserService {
  ...
  async list(filter: Filter): Promise<Data<IUser>> {
    let body = {
      query:
        'query list ($filter: Filter!) { users(filter: $filter) { count rows { id role first_name last_name email phone avatar status createdat creator { id first_name last_name email phone } updatedat updater { id first_name last_name email phone } } } }',
      variables: {
        filter: {
          ...filter,
        },
  ...
  async get(id: String) {
    let body = {
      query:
        'query get($id: UUID!) { user (id: $id) { id role first_name last_name email phone avatar status createdat creator { id first_name last_name email phone } updatedat updater { id first_name last_name email phone } } }',
      variables: {
        id: id,
      },
    };
