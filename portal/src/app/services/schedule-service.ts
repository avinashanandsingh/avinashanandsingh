...
export class ScheduleService {
  ...
  async get(id: String) {
    let body = {
      query:
        'query get($id: UUID!) { schedule (id: $id) { id courseid course { id title } title start_date end_date start_time end_time deadline capacity enrollment_count status } }',
      variables: {
        id: id,
      },
    };
  ...
  async changeStatus(id: string, status: string) {
    let body = {
      query:
        'mutation changeStatus($id: UUID!, $status: Status!) { changeScheduleStatus (id: $id, status: $status) { id } }',
      variables: {
        id: id,
        status: status,
      },
