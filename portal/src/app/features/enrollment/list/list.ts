...
export default class List implements OnInit {
  ...
  async load_course_list(): Promise<void> {
    ...
    if (result.rows) {
      result.rows.forEach((row) => {
        list.push({ id: row.id!, value: row.id!, label: row.title });
      });
    }
    this.course_list.set(list);
  }

  async load_schedule_list(courseId: string): Promise<void> {
    ...
    if (result.rows) {
      result.rows.forEach((row) => {
        list.push({ id: row.id!, value: row.id!, label: `${row.title} - ${row.status}`  });
      });
    }
  ...
  async courseHandler(item: IListItem): Promise<void> {
    ...
    if (item) {
      this.loader.set(true);
      await this.load_schedule_list(item?.value);
      this.loader.set(false);
    }
  }
