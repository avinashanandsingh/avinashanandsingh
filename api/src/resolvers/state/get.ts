...
export default async (_: any, args: any, ctx: any): Promise<Partial<State> | null> => {
  let row: Partial<State> | null = null;
  let table = "view_states";
  ...
  if (result?.count! > 0) {
    row = result?.rows?.shift() as Partial<State>;
  } else {
